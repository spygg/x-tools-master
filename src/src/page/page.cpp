...
void Page::outputText(const QByteArray& bytes, const QString& flag, bool isRx)
{
    ...
    ui->textBrowserOutput->setTextCursor(cursor);

    // 弃用 append，改用 cursor.insertBlock() + cursor.insertHtml()
    cursor = ui->textBrowserOutput->textCursor(); // 重新获取一次确保位置同步
    cursor.insertBlock(); // 强制定位到新的文本块
    cursor.insertHtml(outputText); // 在新文本块中插入 HTML
    ui->textBrowserOutput->setTextCursor(cursor); // 可选，保持光标在最末尾
}
